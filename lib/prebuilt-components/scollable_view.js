Mobile.Component.create('ScrollableView', {
  'class': new Class({
    ...
    current_page: function() {
      ...
      var two_ago = -1, one_ago = -1, scroll = this.scroller.getScroll().y;
      ...
      while(true) {
        ...
        one_ago = guess;
        two_ago = one_ago;
        // Scroll should be somewhere between the top of this frame
        //  And the top of the next frame
        // If the scroll is less than the top of this frame, then we need to move forward one
        if (scroll < this.all_pages[guess].top)
          guess--;
        // If the scroll is greater then the top of the next grame, then we need to move back one
        else if (guess+1 < this.all_pages.length && scroll > this.all_pages[guess+1].top)
          guess++;
        else
          break;
      }
    ...
    Page: new Class({
      ...
      load: function(thenShow) {
        ...
        console.log(this.owner.get_more_at + this.owner.page_name + "=" + (page) + "&" + this.owner.per_page_name + "=" + this.owner.per_page);
        this.loader.send({
      ...
      showDOM: function() {
        ...
        this.top = this.container.getPosition().y;
      },
      ...
      serialize: function() {
        return { data: this.data, container: this.container, top: this.top, page_number: this.page_number };
      },
