Router = new new Class({
  ...
  routeTo: function(url, full_url) {
    ...
    if (!route.action.var_names) route.action.var_names = [];
 
    // Gather the params from the match
    for (var i = 0; i < matches.length; i++)
      if (i < route.action.var_names.length)
        params[route.action.var_names[i]] = matches[i];
