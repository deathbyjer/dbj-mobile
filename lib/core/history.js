...
this.History = new new Class({
	...
	replace: hasPushState ? function(url, title, state, force){
		history.replaceState(state || null, title || null, url);
		this.onChange(url, state, force);
	} : function(url){
		this.hash = '#' + url;
		this.push(url);
	},
	...
	onChange: function(url, state, force){
		this.fireEvent('change', [url, state || {}, force || false]);
	},
