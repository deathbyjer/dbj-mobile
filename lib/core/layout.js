...
Mobile.Layout = new Class({
  ...
  getComponentContainer: function() {
    ...
    if (layout && layout.frames && layouts.frames.length > 1) {
  ...
  replaceComponent: function(component) {
    ...
    Mobile.SwapComponents.start(old_container, component.getFullContainer(), frame.getChildren('.mobile-frame-area')[0], transition_options);
  }
});
...
Mobile.Layout.Loader = new Class({
  ...
  initialize: function(layout) {
    ...
    if (this.layout.loaders)
      for (var i = 0; i < this.layout.loaders.length; i++)
